...
const getQuestionAndAnswer = async (employee) => {
            ...
            switch (answers.role) {
                case "Manager":
                    getEmployeeRoleInfo(answers.role).then(answer => {
                        ...
                        resolved(employee.push(new Manager(answers.id, answers.name, answers.email, moreInfo)));
                    });
                    break;
                case "Engineer":
                    getEmployeeRoleInfo(answers.role).then(answer => {
                        moreInfo = answer.name;
                        resolved(employee.push(new Engineer(answers.id, answers.name, answers.email, moreInfo)));
                    })
                ...
                case "Intern":
                    getEmployeeRoleInfo(answers.role).then(answer => {
                        moreInfo = answer.name;
                        resolved(employee.push(new Intern(answers.id, answers.name, answers.email, moreInfo)));
                    });
                    break;
            }
...
const getInfoFromUser = async () => {
    ...
    while (isNewMemberRequired) {
        await getQuestionAndAnswer(employee);
...
const getHtml =  () => {
    ...
    for (const key in employee) {
        if (employee[key].getRole() == "Manager") {
            htmlofEmployee = readHtmlFile("./src/manager.html", employee[key]);
        } else if (employee[key].getRole() == "Intern") {
            htmlofEmployee = readHtmlFile("./src/intern.html", employee[key]);
        } else if (employee[key].getRole() == "Engineer") {
            htmlofEmployee = readHtmlFile("./src/engineer.html", employee[key]);
        };
